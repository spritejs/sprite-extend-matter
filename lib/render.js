...
Render.world = function (render) {
  ...
  if (options.hasBounds) {
    ...
    for (var _i = 0; _i < allConstraints.length; _i++) {
      var constraint = allConstraints[_i],
  ...
  if (!render.extraDraw) {
    var extraDraw = function extraDraw() {
  ...
  for (var i = 0; i < bodies.length; i++) {
    ...
    for (var k = body.parts.length > 1 ? 1 : 0; k < body.parts.length; k++) {
      ...
      if (s) {
        ...
        var _part4 = part,
            vertices = _part4.vertices;
        var _vertices$2 = vertices[0],
            x0 = _vertices$2.x,
            y0 = _vertices$2.y;
...
Render.bodyConvexHulls = function (render, bodies, context) {
  ...
  for (var i = 0; i < bodies.length; i++) {
    ...
    if (s) {
      ...
      var _body2 = body,
          vertices = _body2.vertices;
      var _vertices$3 = vertices[0],
          x0 = _vertices$3.x,
          y0 = _vertices$3.y;

      var d = 'M0,0';
      for (var j = 1; j < vertices.length; j++) {
        var x = vertices[j].x - x0,
            y = vertices[j].y - y0;
        d += 'L' + x + ',' + y;
      }
      d += 'z';
...
Render.vertexNumbers = function (render, bodies, context) {
  ...
  for (var i = 0; i < bodies.length; i++) {
    ...
    for (var k = parts.length > 1 ? 1 : 0; k < parts.length; k++) {
      var part = parts[k];
...
Render.bodyAxes = function (render, bodies, context) {
  ...
  for (var i = 0; i < bodies.length; i++) {
    ...
    if (!body.render.visible) {
      ...
      for (var j = parts.length > 1 ? 1 : 0; j < parts.length; j++) {
        part = parts[j];
        for (var k = 0; k < part.axes.length; k++) {
      ...
      for (var _j = parts.length > 1 ? 1 : 0; _j < parts.length; _j++) {
        part = parts[_j];
        for (var _k = 0; _k < part.axes.length; _k++) {
